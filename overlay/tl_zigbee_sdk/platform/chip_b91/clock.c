...
/* BRINGUP: PLL lock-telemetry captured inside clock_init_ram's bounded wait.
 * Read after clock_init returns (once 0x81 bit6 is cleared, 0x88 bit5 no longer
 * reports lock, so these are saved before that). Initialized here so a toolchain
 * with partial linking places them in .bss, not random flash. */
unsigned char g_brPlldiagPre = 0xFF;
unsigned char g_brPlldiagPost = 0xFF;
unsigned char g_brPlldiagLocked = 0x00;
                                                      ...
                                                      sys_pll_div_to_mspi_clk_e mspi_clk_div)
{
    ...
    //wait for PLL stable
    //BRINGUP (temporary): bounded wait - infinite hang tells nothing
    {
        g_brPlldiagPre = analog_read_reg8(0x88);
        g_brPlldiagPost = 0xFF;
        g_brPlldiagLocked = 0;
    }
    analog_write_reg8(0x81, (analog_read_reg8(0x81) | BIT(6)));
    {
        unsigned int __to = 0;
        unsigned char __r88 = 0;
        __r88 = analog_read_reg8(0x88); /* first read after lock-detect enable */
        g_brPlldiagPost = __r88;
        g_brPlldiagLocked = (BIT(5) == (__r88 & BIT(5))) ? 1 : 0;
        while ((BIT(5) != (analog_read_reg8(0x88) & BIT(5))) && (__to < 2000000)) {
            __to++;
        }
        g_brPlldiagPost = analog_read_reg8(0x88);
        g_brPlldiagLocked = (BIT(5) == (g_brPlldiagPost & BIT(5))) ? 1 : 0;
    }
    {
        unsigned char __locked = (analog_read_reg8(0x88) & BIT(5));
        analog_write_reg8(0x81, (analog_read_reg8(0x81) & ~BIT(6)));
        if (!__locked) {
            /* PLL did not lock (crystal not oscillating?). Stay on 24M RC
             * instead of switching cclk to an unlocked PLL (which garbles
             * UART and hangs radio). Record real clocks so UART dividers
             * computed from sys_clk.pclk stay correct. */
            write_reg8(0x1401d8, read_reg8(0x1401d8) & 0xfc); // pclk DIV1
            write_reg8(0x1401d8, read_reg8(0x1401d8) & ~BIT(2)); // hclk DIV1
            write_reg8(0x1401e8, read_reg8(0x1401e8) & 0x0f); // cclk/mspi to 24M RC
            sys_clk.cclk = 24;
            sys_clk.hclk = 24;
            sys_clk.pclk = 24;
            sys_clk.mspi_clk = 24;
