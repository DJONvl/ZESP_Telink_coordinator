...
static void sampleGW_zclReadRspCmd(zclIncoming_t *pInMsg)
{
    ...
    *pBuf++ = pReadRspCmd->numAttr;
    ...
    // 0x8100 (native attr-read rsp) DELIBERATELY DISABLED: the response now
    // goes out ONLY through the 0x8902 tunnel, matching the original Yandex
    // gateway. Emitting both made hosts with a race-prone ReadAttribWait see
    // raw hex (with the ZCL string length prefix) instead of the decoded
    // string, depending on which frame won the socket race.
...
static void sampleGW_zclReportCmd(zclIncoming_t *pInMsg)
{
    ...
    *pData++ = pReportCmd->numAttr;
    ...
    /* NOTE: 0x8104 is intentionally not sent. Every incoming frame
     * (including attribute reports) is mirrored to the host through the
     * raw 0x8902 tunnel (see zb_tunnel.c) – sending both would deliver
     * each report twice. Matches the reference gateway firmware, which
     * never emits 0x8104. */
#if 0
