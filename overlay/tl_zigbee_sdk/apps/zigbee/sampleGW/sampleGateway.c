...
{
    /* Pin the coordinator IEEE (see zb_ieee.c): runs after stack_init(),
     * before bdb_init(), every boot. */
    zb_ieeeApplyFixed();
    ...
    /* Register endPoint: all incoming APS frames go through the raw
     * tunnel hook first (0x8902 mirror), then normal ZCL processing. */
    af_endpointRegister(SAMPLE_GW_ENDPOINT, (af_simple_descriptor_t *)&sampleGW_simpleDesc, sampleGW_tunnelRxHandler, sampleGW_dataSendConfirm);
