...
extern void zbhci_ieeeSetHandle(void *arg);
...
static void zbhciMgmtLqiLocalRsp(u8 startIndex)
{
    u8 total = tl_zbNeighborTableNumGet();
    u8 buf[5 + 2 * sizeof(neighbor_tbl_lst_t)] = {0};
    zdo_mgmt_lqi_resp_t *rsp = (zdo_mgmt_lqi_resp_t *)buf;

    rsp->seq_num = 0;
    rsp->status = 0;
    rsp->neighbor_tbl_entries = total;
    rsp->start_index = startIndex;
    rsp->neighbor_tbl_lst_cnt = 0;

    if (startIndex < total) {
        u8 cap = total - startIndex;
        if (cap > 2) {
            cap = 2;
        }
        rsp->neighbor_tbl_lst_cnt = cap;

        for (u8 i = 0; i < cap; i++) {
            tl_zb_normal_neighbor_entry_t *entry = tl_zbNeighborEntryGetFromIdx(startIndex + i);
            if (entry == NULL) {
                rsp->neighbor_tbl_lst_cnt = i;
                break;
            }
            neighbor_tbl_lst_t *rec = &rsp->neighbor_tbl_lst[i];
            memcpy(rec->ext_pan_id, NWK_NIB().extPANId, 8);
            tl_zbExtAddrByIdx(entry->addrmapIdx, rec->ext_addr);
            rec->network_addr = tl_zbshortAddrByIdx(entry->addrmapIdx);
            rec->deviceType = entry->deviceType;
            rec->rxOnWhenIdle = entry->rxOnWhileIdle ? 1 : 0;
            rec->relationship = entry->relationship;
            if (rec->relationship > 3) {
                rec->relationship = 3;
            }
            rec->permitJoining = 0;
            rec->reserved1 = 0;
    ...
    ind.zpdu = buf;
    ind.src_addr = NIB_NETWORK_ADDRESS();
    ind.clusterId = MGMT_LQI_RSP_CLID;
    ind.seq_num = 0;
    ind.status = 0;
    ind.length = 5 + rsp->neighbor_tbl_lst_cnt * sizeof(neighbor_tbl_lst_t);
    ind.relayCmd = false;

    zbhciMgmtLqiRspMsgPush(&ind);
}
...
static void zbhci_mgmtCmdHandler(void *arg)
{
    ...
    if (cmdID == ZBHCI_CMD_MGMT_LQI_REQ) {
        ...
        req.start_index = p[2];
        if (dstAddr == NIB_NETWORK_ADDRESS()) {
            zbhciMgmtLqiLocalRsp(req.start_index);
        } else {
...
/*********************************************************************
 * @fn      zbhci_apsSendHandle
 *
 * @brief   Raw APS passthrough (0x0901). The host builds the whole ASDU
 *          (e.g. manufacturer-specific ZCL frames for 0xFCC0 / 0xEF00).
 *          Request:  mode(1) + addr + srcEp(1) + dstEp(1) + profileBE(2) +
 *                    clusterBE(2) + lenBE(2) + asdu.
 *          Response: 0x8901 [status, apsCounter]. The 0x8200 data-confirm
 *          follows automatically via sampleGW_dataSendConfirm.
 *
 * @param   arg - zbhci_cmdHandler_t
 *
 * @return  None
 */
static void zbhci_apsSendHandle(void *arg)
{
    zbhci_cmdHandler_t *cmdInfo = (zbhci_cmdHandler_t *)arg;
    u8 *p = cmdInfo->payload;
    u16 totalLen = cmdInfo->payloadLen;
    u8 rsp[2] = {0, 0};

    /* Only 16-bit short addressing is supported (host always sends mode 2). */
    if (totalLen >= 11 && p[0] == ZBHCI_ADDRMODE_SHORT) {
        epInfo_t dstEpInfo;
        TL_SETSTRUCTCONTENT(dstEpInfo, 0);
        dstEpInfo.dstAddrMode = APS_SHORT_DSTADDR_WITHEP;
        COPY_BUFFERTOU16_BE(dstEpInfo.dstAddr.shortAddr, &p[1]);
        u8 srcEp = p[3];
        dstEpInfo.dstEp = p[4];
        COPY_BUFFERTOU16_BE(dstEpInfo.profileId, &p[5]);
        u16 clusterId = 0;
        COPY_BUFFERTOU16_BE(clusterId, &p[7]);
        u16 asduLen = 0;
        COPY_BUFFERTOU16_BE(asduLen, &p[9]);

        if ((u32)11 + asduLen <= totalLen) {
            dstEpInfo.txOptions |= APS_TX_OPT_ACK_TX;
            u8 apsCnt = 0;
            rsp[0] = af_dataSend(srcEp, &dstEpInfo, clusterId, asduLen, &p[11], &apsCnt);
            rsp[1] = apsCnt;
        } else {
    ...
    zbhciTx(ZBHCI_CMD_APS_SEND_RSP, sizeof(rsp), rsp);
    ev_buf_free(arg);
}

/*********************************************************************
 * @fn      zbhci_tunnelMsgRecvPush
 *
 * @brief   Mirror an incoming APS message to the host (0x8902):
 *          srcMode(1) + srcBE(2) + dstMode(1) + dstBE(2) + srcEp + dstEp +
 *          profileBE(2) + clusterBE(2) + lenBE(2) + asdu + lqi + rssi.
 *
 * @param   arg - apsdeDataInd_t
 *
 * @return  None
 */
void zbhci_tunnelMsgRecvPush(void *arg)
{
    apsdeDataInd_t *pInd = (apsdeDataInd_t *)arg;

    u16 total = 14 + pInd->asduLen + 2;
    u8 *buf = ev_buf_allocate(total);
    if (buf) {
        u8 *pBuf = buf;
        *pBuf++ = ZBHCI_ADDRMODE_SHORT;
        COPY_U16TOBUFFER_BE(pBuf, pInd->indInfo.src_short_addr);
        pBuf += 2;
        *pBuf++ = ZBHCI_ADDRMODE_SHORT;
        COPY_U16TOBUFFER_BE(pBuf, pInd->indInfo.dst_addr);
        pBuf += 2;
        *pBuf++ = pInd->indInfo.src_ep;
        *pBuf++ = pInd->indInfo.dst_ep;
        COPY_U16TOBUFFER_BE(pBuf, pInd->indInfo.profile_id);
        pBuf += 2;
        COPY_U16TOBUFFER_BE(pBuf, pInd->indInfo.cluster_id);
        pBuf += 2;
        COPY_U16TOBUFFER_BE(pBuf, pInd->asduLen);
        pBuf += 2;
        memcpy(pBuf, pInd->asdu, pInd->asduLen);
        pBuf += pInd->asduLen;
        *pBuf++ = pInd->indInfo.lqi;
        *pBuf++ = pInd->indInfo.rssi;

        zbhciTx(ZBHCI_CMD_APS_MSG_RECV_IND, total, buf);
        ev_buf_free(buf);
    }
}
...
void zbhciCmdHandler(u16 msgType, u16 msgLen, u8 *p)
{
    ...
    if (cmdInfo) {
        ...
        case ZBHCI_CMD_APS_SEND_REQ:
            TL_SCHEDULE_TASK(zbhci_apsSendHandle, cmdInfo);
            break;

        case ZBHCI_CMD_MAC_SET_EXT_ADDR_REQ:
            TL_SCHEDULE_TASK(zbhci_ieeeSetHandle, cmdInfo);
            break;
