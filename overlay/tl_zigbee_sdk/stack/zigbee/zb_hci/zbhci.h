...
typedef enum {
    ...
    /* Raw APS passthrough (host builds/parses the ZCL frame itself).
     * Used for manufacturer-specific clusters (e.g. 0xFCC0, 0xEF00). */
    ZBHCI_CMD_APS_SEND_REQ                      = 0x0901,

    ZBHCI_CMD_APS_SEND_RSP                      = 0x8901,
    ZBHCI_CMD_APS_MSG_RECV_IND                  = 0x8902,

    /* Coordinator IEEE control (custom, see apps/zigbee/sampleGW/zb_ieee.c):
     * boot applies a fixed IEEE (stable identity across NVRAM wipes);
     * 0x0903 sets it at runtime (payload: 8 bytes, display order). */
    ZBHCI_CMD_MAC_SET_EXT_ADDR_REQ              = 0x0903,

    ZBHCI_CMD_MAC_SET_EXT_ADDR_RSP              = 0x8903,
} teHCI_MsgType;
