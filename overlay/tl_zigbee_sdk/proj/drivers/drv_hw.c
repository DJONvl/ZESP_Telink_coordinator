...
static void internalFlashSizeCheck(void)
{
    ...
    if ((pMid[2] < FLASH_SIZE_512K) || \
         ...
         * NOTE (module bringup): small-flash modules (e.g. 256KB) hang here
         * forever. Continue anyway – MAC/NV addresses may be off, but the
         * system boots and talks. */
    }
...
#elif defined(MCU_CORE_B91)
    /* BRINGUP: give slow-starting 24M crystal extra time before sys_init.
     * Without this, pm sees XTAL_UNSTABLE -> reboot or falls back to RC24M. */
    pm_set_xtal_stable_timer_param(1000, 1, 100);
    sys_init(LDO_1P4_LDO_1P8, VBAT_MAX_VALUE_LESS_THAN_3V6, INTERNAL_CAP_XTAL24M);
#elif defined(MCU_CORE_TL321X)
...
#elif defined(MCU_CORE_B91) || defined(MCU_CORE_TL321X) || defined(MCU_CORE_TL323X) || \
      defined(MCU_CORE_TL521X)
    /* STOCK (no C5/C6 speculative writes): plain CLOCK_INIT. */
    CLOCK_INIT;
#endif
